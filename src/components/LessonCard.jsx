import { FiClock, FiEdit2, FiTrash2, FiVideo, FiMapPin, FiCheckCircle, FiXCircle, FiDollarSign } from 'react-icons/fi';
...
function LessonCard({ lesson, liveStatus, onTogglePayment, onToggleCompleted, onDeleteLesson, onEditLesson, isDarkMode }) {
  ...
  // 2. ЛОГІКА СТАТУСУ (Оплачено / Проведено)
  ...
  // Визначаємо, чи активний урок прямо зараз
  ...
  return (
    ...
    >
      {/* --- ЖИВИЙ ТАЙМЕР (Показується тільки якщо урок іде) --- */}
      {isActive && (
        <div style={{ 
          ...
          <div style={{ 
            ...
            width: `${liveStatus.progress}%`, transition: 'width 60s linear' 
          }} />
        </div>
      )}
      ...
      <div className="lc-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', marginTop: isActive ? '5px' : '0' }}>
        <div className="lc-time" style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '18px', fontWeight: 'bold', color: isDarkMode ? '#f3f4f6' : '#1f2937' }}>
        ...
        <div style={{ display: 'flex', gap: '5px' }}>
          <button onClick={() => onEditLesson(lesson)} className="icon-btn edit-btn" title="Редагувати">
            <FiEdit2 size={16} />
          </button>
          <button onClick={() => onDeleteLesson(lesson.id)} className="icon-btn delete-btn" title="Видалити">
            <FiTrash2 size={16} />
          </button>
        </div>
      </div>

      {/* Повідомлення, якщо урок скоро */}
      {isNear && (
        <div style={{ 
          fontSize: '11px', backgroundColor: '#fef3c7', color: '#92400e', 
      ...
      <div style={{ marginBottom: '15px' }}>
        <h3 style={{ margin: '0 0 5px 0', fontSize: '16px', color: isDarkMode ? '#f3f4f6' : '#111827' }}>
          {student.name} <span style={{ fontSize: '13px', color: isDarkMode ? '#9ca3af' : '#6b7280', fontWeight: 'normal' }}>({student.grade} кл.)</span>
        </h3>
        <div style={{ fontSize: '14px', color: isDarkMode ? '#d1d5db' : '#4b5563', display: 'flex', alignItems: 'center', gap: '5px' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
        ...
        <button 
          ...
          style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '5px 10px', borderRadius: '8px', fontSize: '12px', fontWeight: 'bold', border: 'none', cursor: 'pointer', backgroundColor: statusBg, color: statusColor, transition: '0.2s' }}
        >
